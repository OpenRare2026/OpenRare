...
PIPELINE_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, os.pardir, os.pardir, os.pardir))
...
def _default_data_dir() -> str:
    ...
    if env:
        ...
        if not os.path.isabs(raw_root):
            raw_root = os.path.join(PIPELINE_ROOT, raw_root)
        path = os.path.join(raw_root, "vep_data", "hpo_tpm")
    if not os.path.isabs(path):
        path = os.path.join(PIPELINE_ROOT, path)
    return path
