...
RESPONSE_PATH_BASE = os.path.abspath(os.environ.get("RARE_PPI_RESPONSE_PATH_BASE", PROJECT_DIR))
...
def _io_path(path: Any) -> str:
    """Resolve a request/output path for local file IO."""
    return str(Path(str(path)).expanduser().resolve())


def _response_path(path: Any) -> Any:
    """Return paths relative to the service root for API responses."""
    if path is None:
        return None
    try:
        return os.path.relpath(_io_path(path), RESPONSE_PATH_BASE).replace(os.sep, "/")
    except Exception:
        return path


def _response_config_paths(payload: Dict[str, Any]) -> Dict[str, Any]:
    out = dict(payload)
    for key, value in list(out.items()):
        if key in {"DATA_DIR", "CACHE_DIR"} or key.startswith("PATH_"):
            out[key] = _response_path(value)
    return out
...
def _run_score(params: RunParameters) -> Dict[str, Any]:
    ...
    csv_path = _io_path(params.output_csv or _default_csv_path())
    audit_json_path = _io_path(params.audit_json) if params.audit_json else None
    ...
    if audit_json_path:
        os.makedirs(os.path.dirname(os.path.abspath(audit_json_path)), exist_ok=True)
    ...
    payload = {
        "count": int(len(df)),
        "output_format": "csv",
        "csv_path": _response_path(csv_path),
        "columns": list(df.columns),
    }
    if params.include_audit:
        payload["audit"] = audit
    if audit_json_path:
        payload["audit_json"] = _response_path(audit_json_path)
    return payload
...
def _run_score_job(job_id: str, params: RunParameters, input_dir: Optional[str] = None):
    try:
        ...
        if input_dir:
            result["input_dir"] = _response_path(input_dir)
        with _job_lock:
...
def _run_clean_case(params: CleanCaseParameters, job_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    output_csv = Path(_io_path(params.output_csv or _default_clean_case_csv_path()))
    ppi_output_csv = Path(_io_path(params.ppi_output_csv)) if params.ppi_output_csv else derive_ppi_output_csv(output_csv)
    ...
    payload = {
        ...
        "count": int(len(final)),
        "output_format": "csv",
        "csv_path": _response_path(output_csv),
        "ppi_csv_path": _response_path(ppi_output_csv),
...
def health():
    ...
    return _completion(
        data_dir=_response_path(cfg.DATA_DIR),
...
def get_config(include_paths: bool = True, include_missing_files: bool = False):
    cfg = Config()
    payload = _response_config_paths(cfg.to_dict(include_paths=include_paths))
    if include_missing_files:
        payload["missing_files"] = [_response_path(path) for path in cfg.missing_files()]
...
async def initialize(params: RunParameters | None = None):
    ...
    try:
        ...
        return _completion(data_dir=_response_path(cfg.DATA_DIR))
    except Exception as exc:
...
async def score_upload(
    ...
    try:
        ...
        result = await run_in_threadpool(_run_score, params)
        return _completion(input_dir=_response_path(input_dir), **result)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
...
async def score_upload_async(
    ...
    job_id = str(uuid.uuid4())
    with _job_lock:
        _jobs[job_id] = {"job_id": job_id, "status": STATUS_QUEUING, "input_dir": _response_path(input_dir)}
    _job_executor.submit(_run_score_job, job_id, params, input_dir)
    return {"job_id": job_id, "status": STATUS_QUEUING, "input_dir": _response_path(input_dir)}
...
async def score_clean_case_upload_async(
    ...
    with _job_lock:
        _jobs[job_id] = {
            ...
            "mode": "clean_case",
            "input_dir": _response_path(input_dir),
        }
    _job_executor.submit(_run_clean_case_job, job_id, params, input_dir)
    return {"job_id": job_id, "status": STATUS_QUEUING, "mode": "clean_case", "input_dir": _response_path(input_dir)}
...
def download_score_csv(job_id: str):
    with _job_lock:
        ...
        csv_path = _io_path(job.get("csv_path"))
...
def download_score_ppi_csv(job_id: str):
    with _job_lock:
        ...
        if job.get("status") == STATUS_FAILURE:
            raise HTTPException(status_code=400, detail=job.get("message", "任务失败"))
        csv_path = _io_path(job.get("ppi_csv_path"))
    if not csv_path or not os.path.exists(csv_path):
