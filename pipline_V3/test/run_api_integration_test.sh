...
source "${REPO_ROOT}/pipline_V3/config/paths.sh"
...
prepare_pipeline_input() {
  ...
  bash "$REPO_ROOT/pipline_V3/test/prepare_sites_vcf_with_sample.sh" \
...
verify_output() {
  local output_dir="$1"
  local sorted="${output_dir}/06_result_sorting/vep_output.sorted.csv"
