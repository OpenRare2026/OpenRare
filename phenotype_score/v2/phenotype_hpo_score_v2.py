...
DEFAULT_DATA_DIR = Path("/mnt/workspace/xiongliwen/00.PublicData")
DEFAULT_WORK_DIR = Path("/mnt/workspace/xiongliwen/03.phenotype_score/v2")
DEFAULT_INPUT_CSV = Path(
    "/mnt/workspace/pangjiangshuan/vep_runner/output/all_900_genes_mutations1_pass_weak_germline.vep.csv"
)
DEFAULT_HPO_FILE = DEFAULT_WORK_DIR / "hpo_test.txt"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phenotype-HPO score v2")
    p.add_argument("--input-csv", type=Path, default=DEFAULT_INPUT_CSV)
    p.add_argument("--hpo-file", type=Path, default=DEFAULT_HPO_FILE)
    p.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR)
    p.add_argument("--outdir", type=Path, default=DEFAULT_WORK_DIR / "results_v2")
...
def query_omim_sqlite(db_path: Path, genes: set[str], alias_index: dict[str, dict[str, str]]) -> dict[str, list[dict[str, str]]]:
    ...
    for gene in sorted(genes):
        rows = cur.execute(
            ...
            "from omim where hgnc_approved_gene_symbol = ? or hgnc_gene_symbol = ?",
            (gene, gene),
        ).fetchall()
        for r in rows:
...
def load_orphapackets(
    ...
    if log_progress:
        log(
            ...
            f"genes={len(out)}, diseases_with_hpo={len(orpha_hpos)}, gene_disease_records={sum(len(v) for v in orpha_gene_records.values())}"
        )
    return out, orpha_to_omim_gene_refs
...
def main() -> None:
    ...
    hpo_dir = data_dir / "phenotype_hpo_v1"
    hp_obo = hpo_dir / "hp.obo"
    hpoa = hpo_dir / "phenotype.hpoa"
    hgnc = hpo_dir / "hgnc_complete_set.txt"
    genes_to_disease = hpo_dir / "genes_to_disease.txt"
    omim_db = data_dir / "omim" / "omim_20250411.sqlite3"
    orpha_yaml_dir = data_dir / "orphadata" / "Orphapackets" / "orphapacket" / "yaml"
    mondo_obo = data_dir / "MONDO" / "mondo-rare.obo"
    ...
    if "all_genes" not in df.columns:
        raise ValueError("input CSV must contain all_genes column")
    ...
    gene_raw_to_norm: dict[str, tuple[str, str, str]] = {}
    for _, row in df.iterrows():
        raw_genes = split_gene_list(row.get("all_genes", ""))
        # Include gene_symbol for variant-level alignment safety.
        raw_genes.extend(split_gene_list(row.get("gene_symbol", "")))
        for raw_gene in raw_genes:
            gene, hgnc_id, status = normalize_gene(raw_gene, alias_index)
            gene_raw_to_norm[raw_gene] = (gene, hgnc_id, status)
            if gene:
                candidate_genes.add(gene)

    log(f"      Variant rows={len(df)}, candidate genes={len(candidate_genes)} ({elapsed_since(step_started)})")
    ...
    for _, row in df.iterrows():
        seen: set[str] = set()
        for raw_gene in split_gene_list(row.get("all_genes", "")):
    ...
    for row in gene_rows:
        row["candidate_variant_count_in_gene"] = counts.get(row["gene_symbol"], 0)
    ...
    variant_df = pd.concat([df.drop(columns=["_variant_id_v2"]), append_df], axis=1)
    variant_out = args.outdir / "variant_phenotype_score.csv"
    variant_df.to_csv(variant_out, index=False)
    log(f"      Output files written: gene_rows={len(gene_rows)}, variant_rows={len(variant_df)} ({elapsed_since(step_started)})")

    summary = {
        "input_csv": str(args.input_csv),
        "hpo_file": str(args.hpo_file),
        "candidate_genes": len(candidate_genes),
        "scored_genes": sum(1 for r in gene_rows if r.get("gene_score")),
        "variant_rows": len(df),
        "gene_output": str(gene_out),
