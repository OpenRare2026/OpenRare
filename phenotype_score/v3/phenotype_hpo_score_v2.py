...
DEFAULT_INPUT_CSV = DEFAULT_WORK_DIR / "test.P002.csv"
    ...
    if index_path and not max_files and index_path.exists() and not rebuild_index:
        ...
        if log_progress:
            log(
                ...
                f"indexed_genes={len(all_profiles)}, matched_genes={len(filtered)}, path={index_path} ({elapsed_since(step_started)})"
            )
...
def main() -> None:
    ...
    orpha_index = args.orpha_index or (data_dir / "index" / "orpha_gene_profiles.pkl")
