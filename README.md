...
V3 仓库以代码和小型处理后资源为主。大型公共数据库建议按下表下载/准备到固定目录，或在命令行/API 中用高级参数覆盖路径。`pipline_V3_all/` 是工作站上的自包含版本，已把尽可能多的资源复制到 `resource/`，但 GitHub 分支主要提交 `pipline_V3/` 代码目录。
...
- BED/VCF 大文件一般需要 `bgzip` 压缩并用 `tabix` 建索引。
- VCF 染色体命名必须和资源一致，例如同为 `chr1` 或同为 `1`。
- cCRE/ncRNA BED 使用 BED 的 0-based half-open 坐标；VCF 使用 1-based POS，模块内部负责转换。
- API 上传的 VCF 会保存到 job 目录 `input/` 下；未压缩 `.vcf` 会自动压缩到输出目录 `00_input/`。
- GitHub 仓库不建议提交 VEP cache、CADD、SpliceAI、dbNSFP、GTEx 这类大型数据库；生产环境请按上述路径准备或通过参数覆盖。
